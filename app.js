...
function normalizePhoneID(input){
  ...
  if(!s) return "";
  // keep digits and plus, strip spaces/dashes/etc
  s = s.replace(/[^0-9+]/g, "");
  if(s.startsWith("+")){
  ...
  else if(s.startsWith("62")) s = "+62" + s.slice(2);
  else if(!s.startsWith("+")) s = "+" + s;
  // basic length check (digits only)
  const digits = s.replace(/\D/g, "");
  if(digits.length < 10 || digits.length > 15) return s; // still return, backend will validate too
  return s;
}

function isValidPhoneID(p){
  ...
  function normalizeEndpoint(endpoint){
    ...
    // Lowercase only the first segment (route name) to avoid 404 due to casing
  ...
  async function neura(endpoint){
    ...
    const r = await apiCall("neura_proxy", { endpoint: ep });
    return r.data;
  }
  ...
  function renderMembers(){
    ...
    const rows = list.map(m=>{
      ...
      const privateLine = showPrivate
        ? '<div class="imeta">Alias: '+escapeHtml(m.name||"-")+' | HP: '+escapeHtml(m.phone||"-")+'</div>'
        : "";
  ...
  function renderBannerAva(items){
    const arr = Array.isArray(items) ? items : [];
    if(arr.length===0) return '<div class="note">Banner kosong.</div>';

    const cards = arr.slice(0,24).map(x=>{
      const title = String((x && (x.title||x.name||x.id)) || "Banner");
      const img = String((x && (x.image||x.img||x.thumbnail||x.url)) || "");
      const dateStr = String((x && (x.dateStr||x.date||x.when)) || "");
      const link = String((x && (x.link||x.href||x.url||x.image)) || "");

      const imgHtml = img
        ? ('<a class="bnImgWrap" href="'+escapeHtml(link||img)+'" target="_blank" rel="noopener">'
           + '<img class="bnImg" src="'+escapeHtml(img)+'" alt="'+escapeHtml(title)+'" loading="lazy" />'
           + '</a>')
        : '<div class="bnImgWrap bnPlaceholder">No image</div>';

      const meta = dateStr ? ('<div class="bnMeta">'+escapeHtml(dateStr)+'</div>') : '';
      return (
        '<div class="bnCard">'
        + imgHtml
        + '<div class="bnBody">'
        +   '<div class="bnTitle">'+escapeHtml(title)+'</div>'
        +   meta
        + '</div>'
        + '</div>'
      );
    ...
    return '<div class="bnGrid">'+cards+'</div>'
      + (arr.length>24 ? '<div class="note">Menampilkan 24 banner pertama.</div>' : '');
  }
  ...
  // Settings
  $("settingsBtn").addEventListener("click", ()=>{ if(!isAdmin) return; openSettings(); });
  $("settingsClose").addEventListener("click", closeSettings);
  settingsModal.addEventListener("click", (e)=>{ if(e.target===settingsModal) closeSettings(); });

  $("settingsSave").addEventListener("click", ()=>{
    if(!isAdmin){ alertBox("settingsAlert","Hanya admin yang bisa set URL","err"); return; }
  ...
  // auto-normalize phone input to +62 format
  if($("mPhone")){
    $("mPhone").addEventListener("blur", ()=>{
      const v = $("mPhone").value.trim();
      if(v) $("mPhone").value = normalizePhoneID(v);
    });
  }
  ...
  $("mAdd").addEventListener("click", async ()=>{
    ...
    if(!phoneRaw){ alertBox("membersAlert","No HP wajib diisi","err"); return; }
    const phone = normalizePhoneID(phoneRaw);
    if(!phone || !isValidPhoneID(phone)){ alertBox("membersAlert","No HP tidak valid","err"); return; }
    $("mPhone").value = phone
  ...
  $("loadBanner").addEventListener("click", async ()=>{
    ...
    try{
      ...
      if(Array.isArray(list)){
        $("bannerBox").innerHTML = renderBannerAva(list);
      }else{
  ...
  $("loadLv").addEventListener("click", async ()=>{
    ...
    if(!cur || !rng){
      $("lvBox").innerHTML = '<div class="alert err">Isi current & range</div>';
      return;
    }
    try{
      setBusy(btn, true, "Cari…");
      ...
      // Neura payloads sometimes use "result" or "results"
      const resultsText =
        (data && typeof data === "object" && typeof data.results === "string") ? data.results :
        (data && typeof data === "object" && typeof data.result  === "string") ? data.result  :
        (typeof data === "string" ? data : "");

      if(resultsText){
        const target = (data && typeof data==="object" && ("level_target" in data)) ? String(data.level_target) : cur;
        const gap = (data && typeof data==="object" && ("gap" in data)) ? String(data.gap) : rng;

        $("lvBox").innerHTML =
          '<div class="lvSummary">Target: Lv <b>'+escapeHtml(target)+'</b> • Gap: <b>'+escapeHtml(gap)+'</b></div>'
          + '<div class="pre pre-lv">'+escapeHtml(normalizeTextBlock(resultsText))+'</div>';
      }else{
        // Fallback: show JSON (trimmed)
        const pretty = JSON.stringify(data, null, 2);
        $("lvBox").innerHTML = '<div class="pre pre-lv">'+escapeHtml(pretty)+'</div>';
      }
    }catch(err){
    ...
    try{
      ...
      if(Array.isArray(list)){
        $("neuraOut").innerHTML =
          ...
          + '<div class="pre pre-neura">'+escapeHtml(JSON.stringify(list.slice(0,15),null,2))+'</div>'
          + '<div class="note">(Menampilkan 15 pertama)</div>';
      }else{
        $("neuraOut").innerHTML = '<div class="pre pre-neura">'+escapeHtml(JSON.stringify(data,null,2))+'</div>';
      }
    }catch(err){
      if(String(err.message||"").indexOf("Neura HTTP 404")>=0){
        $("neuraOut").innerHTML =
          '<div class="alert err">Gagal: '+escapeHtml(err.message)+'</div>'
          + '<div class="note">Tips: endpoint harus lowercase. Contoh: <b>xtal</b>, <b>buff</b>, <b>ability</b>, <b>item</b>, <b>lv/current=50&range=5</b>.</div>';
      }else{
